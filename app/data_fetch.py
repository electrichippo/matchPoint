...
def get_match_data_for_tournament(competition_id, tournamentName):
    ...
    """
    pageNum = 0
    base_match_api_url = "https://api.au.pointsbet.com/api/mes/v3/events/featured/competition/{competition_id}?page={pageNum}"
    headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.110 Safari/537.3'}
    all_match_data = []
    count = 0
    ...
    while 1:
        if competition_id:
            pageNum+=1
            match_api_url = base_match_api_url.format(competition_id=competition_id, pageNum = str(pageNum))
            print(match_api_url)
            ...
            try:
                ...
                if len(match_data_list) == 0: break
                simplified_matches = []
                ...
                print(f"Error fetching match data for {tournamentName} (ID: {competition_id}): {e} page {pageNum}")
            except json.JSONDecodeError as e:
...
def data_to_json(competitionDetails):

    all_matches = []
    for competition in competitionDetails:
        matches = get_data(competition[0], competition[1])
...
if __name__ == "__main__":
    ...
    all_matches = data_to_json([[11481, "French Open Men's"], [11480, "French Open Women's"]])
